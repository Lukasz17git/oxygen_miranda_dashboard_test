...
class Booking {
    ...
    get fee() {
        if (!this.room)
            throw "booking is not assigned to any room, please add it to a room";
        const isInt = (number) => Number.isInteger(number);
        if (!isInt(this.room.price) || !isInt(this.room.discount) || !isInt(this.discount))
            throw "price and discounts should be an integer";
        if (this.room.discount > 100 || this.discount > 100)
            throw 'discounts should be lower than 100';
        if (this.room.discount < 0 || this.discount < 0 || this.room.price < 0)
            throw 'discounts and price cant be negative';
        return Math.round((this.room.price * (100 - this.room.discount) * (100 - this.discount)) / 10000);
    }
    addRoom(room) {
        if (!(room instanceof Room))
            throw `Provided room: ${room} is not an instance of Room class`;
        room.addBooking(this);
        return this;
    }
    removeRoom() {
        if (!this.room)
            throw 'There is no room inside Booking';
...
class Room {
    ...
    addBooking(booking) {
        if (!(booking instanceof Booking))
            throw `Provided booking: ${booking} is not an instance of Booking class`;
        booking.room = this;
    ...
    addBookings(bookings) {
        if (!Array.isArray(bookings))
            throw 'Bookings should be an array of Bookings';
    ...
    removeBooking(identifier) {
        if (typeof identifier === 'number') {
            ...
            throw `wrong identifier to remove a booking, use index or id instead of ${identifier}`;
        return this;
    }
    static calculateStartAndEndDayAsInteger(startDate, endDate, timezoneOffsetInMinutes = 120) {
        // if dates are already as integer day dates, skip and return those
        if (typeof startDate === 'number' && typeof endDate === 'number' && startDate < 100000 && endDate < 100000) {
    ...
    calculateOcupancyMap(startDayAsInteger, endDayAsInteger = startDayAsInteger) {
        ...
        for (const booking of this.bookings) {
            ...
            if (bookingStartDayAsInteger > endDayAsInteger || bookingEndDayAsInteger < startDayAsInteger)
                continue;
