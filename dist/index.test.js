...
test('Room class constructor', () => {
    // Default Room
    const roomDefault = new index_1.Room();
    ...
    // Room 1
    const room1 = new index_1.Room({ name: 'Premium Room', price: 15000, discount: 10 });
    expect(room1.name).toBe('Premium Room');
    expect(room1.price).toBe(15000);
    expect(room1.discount).toBe(10);
    // Room 2
    const room2 = new index_1.Room({ name: 'Premium Room', price: 15000, discount: 120 });
...
test('addBooking method should add a booking and add a property called room to the booking', () => {
    ...
    expect((_a = room.addBooking(booking).bookings.at(-1)) === null || _a === void 0 ? void 0 : _a.room).toBe(room);
});
...
test('addBookings method should add all the bookings and add a property called room to each of the bookings', () => {
    ...
    expect((_a = room.addBookings([booking, booking2]).bookings.at(-1)) === null || _a === void 0 ? void 0 : _a.room).toBe(room);
});
...
test('removeBooking should remove the booking from the room and also set the room property of the booking to null', () => {
    ...
    expect(new index_1.Room().addBooking(booking).removeBooking(0).bookings.length).toBe(0);
    expect(new index_1.Room().addBooking(booking).removeBooking(1).bookings.length).toBe(1);
    expect(new index_1.Room().addBooking(booking).removeBooking(1000).bookings.length).toBe(1);
    expect(new index_1.Room().addBookings([booking, booking2]).removeBooking(-1).bookings.length).toBe(1);
    expect(new index_1.Room().addBookings([booking, booking2]).removeBooking(-1).removeBooking(-1).bookings.length).toBe(0);
    expect(new index_1.Room().addBooking(booking).removeBooking('something').bookings.length).toBe(1);
    //testing the room prop of booking
    const bookingWithoutRoom = new index_1.Booking();
    expect(bookingWithoutRoom.room).toBe(null);
    const roomWithBooking = new index_1.Room().addBooking(bookingWithoutRoom);
...
test('isOccupied without an argument should return true if there is a booking for the current day, false otherwise', () => {
    const room = new index_1.Room();
    room.addBooking(new index_1.Booking({ checkIn: daysAgo(2), checkOut: daysAgo(1) }));
    expect(room.isOccupied()).toBe(false);
    room.addBooking(new index_1.Booking({ checkIn: new Date() }));
...
test('isOccupied will correctly calculate the day of a given date even if this one contains hours, minutes, seconds...', () => {
    const room = new index_1.Room();
    room.addBooking(new index_1.Booking({ checkIn: new Date('2023-06-13T21:21:15.678Z') }));
...
test('isOccupied should calculate the day by the timezone of the hotel (inside settings), even if the manager is logged in completly different timezone', () => {
    const room = new index_1.Room();
    room.addBooking(new index_1.Booking({ checkIn: new Date('2023-06-13T23:21:15.678Z') })); // 13-06 in UTC, but 14-06 in Spain (default settings)
...
test('isOccupied should return true if there is a booking in the selected date and false otherwise', () => {
    const room = new index_1.Room();
    room.addBooking(new index_1.Booking({ checkIn: daysAgo(5), checkOut: daysAgo(2) }));
...
test('isOccupied should return true if there is a booking without checkOut date, where it counts as current running day', () => {
    const room = new index_1.Room();
    room.addBooking(new index_1.Booking({ checkIn: daysAgo(5) }));
    expect(room.isOccupied()).toBe(true);
    expect(room.isOccupied(daysAgo(3))).toBe(true);
    expect(room.isOccupied(daysAgo(7))).toBe(false);
...
test('isOccupied method should return true if a room has a booking with the same dates as checkIn and checkOut', () => {
    const room = new index_1.Room();
    expect(room.isOccupied()).toBe(false);
    const booking = new index_1.Booking({ checkIn: new Date('06-13-2023'), checkOut: new Date('06-13-2023') });
...
test('isOccupied with dates in future should return only true if the checkIn date is the same to the isOccupied argument', () => {
    const room = new index_1.Room();
    room.addBooking(new index_1.Booking({ checkIn: daysInFuture(1) }));
...
test('occupancyPercentage should return a rounded integer, between 0 and 100, where 0 is 0% and 100 is 100% of occupancy and so on', () => {
    ...
    expect(room.occupancyPercentage(daysAgo(2))).toBe(33);
    expect(room.occupancyPercentage(daysAgo(3))).toBe(25);
    const room2 = new index_1.Room();
    room2.addBooking(new index_1.Booking({ checkIn: daysAgo(5), checkOut: daysAgo(2) }));
...
test('totalOccupancyPercentage should return null if there is no rooms provided as argument', () => {
    expect(index_1.Room.totalOccupancyPercentage()).toBe(null);
    //@ts-ignore
    expect(index_1.Room.totalOccupancyPercentage(null, daysAgo(2), daysAgo(1))).toBe(null);
    expect(index_1.Room.totalOccupancyPercentage([])).toBe(null);
});
test('totalOccupancyPercentage should throw an error if there is at least one no-room inside rooms array', () => {
    //@ts-ignore
    expect(() => index_1.Room.totalOccupancyPercentage([{}])).toThrow();
    //@ts-ignore
    expect(() => index_1.Room.totalOccupancyPercentage([new index_1.Room(), {}])).toThrow();
});
test('totalOccupancyPercentage should return 0 if there is no bookings inside rooms', () => {
    const room1 = new index_1.Room();
    const room2 = new index_1.Room();
    const rooms = [room1, room2];
    expect(index_1.Room.totalOccupancyPercentage(rooms)).toBe(0);
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(2))).toBe(0);
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(2), daysAgo(1))).toBe(0);
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(2), daysAgo(5))).toBe(0);
});
test('totalOccupancyPercentage should return the check for current date if there are no dates provided; 0 for empty, 100 for occupied', () => {
    ...
    const rooms = [room];
    expect(index_1.Room.totalOccupancyPercentage(rooms)).toBe(0);
    room.addBooking(new index_1.Booking({ checkIn: daysAgo(1), checkOut: daysAgo(1) }));
    expect(index_1.Room.totalOccupancyPercentage(rooms)).toBe(0);
    room.addBooking(new index_1.Booking({ checkIn: new Date() }));
    expect(index_1.Room.totalOccupancyPercentage(rooms)).toBe(100);
});
test('totalOccupancyPercentage should check until the current date if only starting date is provided', () => {
    const room = new index_1.Room();
    room.addBooking(new index_1.Booking());
    const rooms = [room];
    expect(index_1.Room.totalOccupancyPercentage(rooms)).toBe(100);
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(0))).toBe(100);
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(1))).toBe(50);
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(2))).toBe(33);
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(3))).toBe(25);
});
test('totalOccupancyPercentage will count as 1 day if the starting and ending date is the same', () => {
    const room = new index_1.Room();
    room.addBooking(new index_1.Booking({ checkIn: new Date() }));
    const rooms = [room];
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(1), daysAgo(1))).toBe(0);
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(0), daysAgo(0))).toBe(100);
});
test('totalOccupancyPercentage will count ending date as current date if ending date is less than starting date', () => {
    const room = new index_1.Room();
    room.addBooking(new index_1.Booking({ checkIn: new Date() }));
    const rooms = [room];
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(1), daysAgo(2))).toBe(50);
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(0), daysAgo(2))).toBe(100);
});
test('totalOccupancyPercentage should return a rounded integer, between 0 and 100, where 0 is 0% and 100 is 100% of occupancy between ALL ROOMS, and so on', () => {
    const room1 = new index_1.Room().addBooking(new index_1.Booking({ checkIn: daysAgo(10), checkOut: daysAgo(8) }));
    const room2 = new index_1.Room().addBooking(new index_1.Booking({ checkIn: daysAgo(6), checkOut: daysAgo(4) }));
    const room3 = new index_1.Room().addBooking(new index_1.Booking({ checkIn: daysAgo(1) }));
    const rooms = [room1, room2, room3];
    expect(index_1.Room.totalOccupancyPercentage(rooms)).toBe(33);
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(0))).toBe(33); // 0% + 0% + 100% (1 day)
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(1))).toBe(33); // 0% + 0% + 100% (2 days)
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(2))).toBe(22); // 0% + 0% + 67% (3 days)
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(3))).toBe(17); // 0% + 0% + 50% (4 days)
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(4))).toBe(20); // 20% + 0% + 40% (5 days)
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(5))).toBe(22); // 33% + 0% + 33% (6 days)
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(6))).toBe(24); // 43% + 0% + 29% (7 days)
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(7))).toBe(21); // 38% + 0% + 25% (8 days)
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(8))).toBe(22); // 33% + 11% + 22% (9 days)
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(9))).toBe(23); // 30% + 20% + 20% (10 days)
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(9), daysAgo(8))).toBe(33); // 100% + 0% + 0% 
    expect(index_1.Room.totalOccupancyPercentage(rooms, daysAgo(8), daysAgo(6))).toBe(22); // 33% + 33% + 0% 
});
/**
 *
...
test('availableRooms should return null if there is no rooms provided as argument', () => {
    expect(index_1.Room.availableRooms()).toBe(null);
    //@ts-ignore
    expect(index_1.Room.availableRooms(null, daysAgo(2), daysAgo(1))).toBe(null);
});
test('availableRooms should return [] if there is an empty array', () => {
    const rooms = [];
    expect(index_1.Room.availableRooms(rooms)).toEqual([]);
    expect(index_1.Room.availableRooms(rooms, daysAgo(2))).toEqual([]);
    expect(index_1.Room.availableRooms(rooms, daysAgo(2), daysAgo(1))).toEqual([]);
    expect(index_1.Room.availableRooms(rooms, daysAgo(2), daysAgo(5))).toEqual([]);
});
test('availableRooms should throw an error if there is at least one no-room inside rooms array', () => {
    //@ts-ignore
    expect(() => index_1.Room.availableRooms([{}])).toThrow();
    //@ts-ignore
    expect(() => index_1.Room.availableRooms([new index_1.Room(), {}])).toThrow();
});
test('availableRooms should return the check for current date if there are no dates provided', () => {
    const room1 = new index_1.Room().addBooking(new index_1.Booking({ checkIn: daysAgo(1) }));
    const room2 = new index_1.Room().addBooking(new index_1.Booking({ checkIn: daysAgo(3), checkOut: daysAgo(2) }));
    const rooms = [room1, room2];
    expect(index_1.Room.availableRooms(rooms)).toEqual([room2]);
});
test('availableRooms should check until the current date if only starting date is provided', () => {
    ...
    const rooms = [room1, room2];
    expect(index_1.Room.availableRooms(rooms, daysAgo(2))).toEqual([]);
    expect(index_1.Room.availableRooms(rooms, daysAgo(1))).toEqual([room2]);
});
test('availableRooms will count ending date as current date if ending date is less than starting date', () => {
    const room1 = new index_1.Room().addBooking(new index_1.Booking({ checkIn: daysAgo(1) }));
    const room2 = new index_1.Room().addBooking(new index_1.Booking({ checkIn: daysAgo(3), checkOut: daysAgo(2) }));
    const rooms = [room1, room2];
    expect(index_1.Room.availableRooms(rooms, daysAgo(2), daysAgo(4))).toEqual([]);
    expect(index_1.Room.availableRooms(rooms, daysAgo(1), daysAgo(4))).toEqual([room2]);
});
test('availableRooms will count as 1 day if the starting and ending date is the same', () => {
    const room1 = new index_1.Room().addBooking(new index_1.Booking({ checkIn: daysAgo(1) }));
    const room2 = new index_1.Room().addBooking(new index_1.Booking({ checkIn: daysAgo(3), checkOut: daysAgo(1) }));
    const rooms = [room1, room2];
    expect(index_1.Room.availableRooms(rooms, daysAgo(0), daysAgo(0))).toEqual([room2]);
    expect(index_1.Room.availableRooms(rooms, daysAgo(1), daysAgo(1))).toEqual([]);
});
...
test('availableRooms should return the available rooms if there is an available room across all choosen days', () => {
    ...
    const rooms = [room1, room2];
    expect(index_1.Room.availableRooms(rooms)).toEqual([room2]);
    expect(index_1.Room.availableRooms(rooms, daysAgo(1))).toEqual([]);
    expect(index_1.Room.availableRooms(rooms, daysAgo(2))).toEqual([]);
    expect(index_1.Room.availableRooms(rooms, daysAgo(2), daysAgo(1))).toEqual([]);
    expect(index_1.Room.availableRooms(rooms, daysAgo(3), daysAgo(2))).toEqual([room1]);
    expect(index_1.Room.availableRooms(rooms, daysAgo(8), daysAgo(4))).toEqual([room1, room2]);
});
...
test('get fee() should throw an error if the booking is not asociated with a room', () => {
    const booking = new index_1.Booking({ discount: 50 });
    expect(() => booking.fee).toThrow();
});
test('get fee() should throw an error if the discount is not an integer', () => {
    const booking = new index_1.Booking({ discount: 50.5 });
    //@ts-ignore
    const booking2 = new index_1.Booking({ discount: 'not integer' });
    expect(() => booking.fee).toThrow();
    expect(() => booking2.fee).toThrow();
});
test('get fee() should throw an error if the discount of the room is not an integer', () => {
    const room = new index_1.Room({ discount: 200.20 });
    //@ts-ignore
    const room2 = new index_1.Room({ discount: 'not integer' });
    const booking = new index_1.Booking({ discount: 10 });
    const booking2 = new index_1.Booking({ discount: 10 });
...
test('get fee() should throw an error if the price of the room is not an integer', () => {
    const room = new index_1.Room({ price: 200.20 });
    //@ts-ignore
    const room2 = new index_1.Room({ price: 'not integer' });
    const booking = new index_1.Booking({ discount: 10 });
    const booking2 = new index_1.Booking({ discount: 10 });
    booking.addRoom(room);
    booking2.addRoom(room2);
...
test('get fee() should return the total fee, in cents as integer, for valid values', () => {
    const room = new index_1.Room({ discount: 50, price: 20000 });
    const booking = new index_1.Booking({ discount: 50 });
    booking.addRoom(room);
    expect(booking.fee).toBe(5000);
});
test('get fee() should round the value to an integer and never return a float value', () => {
    const booking = new index_1.Booking({ discount: 5 }).addRoom(new index_1.Room({ price: 1000, discount: 87 })); // price = 1€ * 0.95 * 0.13
    const booking2 = new index_1.Booking({ discount: 13 }).addRoom(new index_1.Room({ price: 100, discount: 27 })); // price = 1€ * 0.87 * 0.73
...
test('get fee() should return 0 if any of the discounts is 100', () => {
    const room = new index_1.Room({ discount: 100, price: 20000 });
    const room2 = new index_1.Room({ discount: 0, price: 20000 });
    const booking = new index_1.Booking({ discount: 50 });
    const booking2 = new index_1.Booking({ discount: 100 });
    booking.addRoom(room);
    booking2.addRoom(room2);
